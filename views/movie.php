...
// Note familiale : une ligne par personne, le detail des visionnages en dessous
// pour qui a note l'oeuvre plusieurs fois.
                ...
                <?php if ($familyRating['average'] !== null): ?>
                    ...
                    <span class="relative" x-data="{ ouvert: false }"
                        ...
                        <button type="button" @click="ouvert = !ouvert"
                            ...
                            ★ <?= Security::e(FormatUtils::score((float) $familyRating['average'])) ?>/5 en famille
                            <span class="text-amber-200/70">(<?= (int) $familyRating['count'] ?>)</span>
                        </button>
                        <span x-show="ouvert" x-cloak
                              class="absolute left-0 top-full z-30 mt-1 w-max min-w-[12rem] rounded-xl bg-slate-800 p-2 text-left shadow-lg ring-1 ring-white/15">
                            <?php foreach ($notesFamille as $note): ?>
                                <span class="block px-1 py-0.5">
                                    <span class="flex items-center gap-2 text-xs text-slate-200">
                                        <?= Avatars::render($note['avatar'], $note['color'], 18) ?>
                                        <span class="flex-1"><?= Security::e($note['name']) ?></span>
                                        <strong class="text-amber-100">
                                            <?= Security::e(FormatUtils::score($note['average'])) ?>/5
                                        </strong>
                                    </span>
                                    <?php if (count($note['scores']) > 1): ?>
                                        <?php /* Deux visionnages ou plus : on montre d'ou vient la moyenne. */ ?>
                                        <span class="ml-7 block text-[11px] text-slate-500">
                                            <?= Security::e(implode(' · ', array_map(
                                                static fn (array $v): string => $v['score'] . '/5 le ' . FormatUtils::shortDate($v['date']),
                                                $note['scores']
                                            ))) ?>
                                        </span>
                                    <?php endif; ?>
