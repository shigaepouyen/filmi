...
- Produces:
  ...
  - `->skip(int $id): void`
  - `->recordChoice(int $seanceId, array $shortlistIds, int $chosenId): void` transactionnelle. Une shortlist **vide est légitime** et signifie qu'il n'y a pas eu de tirage : c'est le cas des semaines filles, qui choisissent dans tout le pool et ne consomment donc aucun créneau de cooldown. Une shortlist non vide doit contenir le film retenu, sinon `InvalidArgumentException`.
...
5. Vider le champ nom et enregistrer : le bandeau rouge « Le nom ne peut pas etre vide. » s'affiche, le profil est inchangé, et **aucune erreur 500** ne remonte. Le dépôt lève une exception sur un nom vide, la page doit l'intercepter en amont.
6. Supprimer le cookie, recharger `/settings.php` : redirection vers le choix de profil.

Expected: les six points passent. Corriger avant de continuer.
